<template>
  <div class="home">
    <!-- 首页第一屏：让评审一眼看出这是“社区服务 + 商城”的居民端门户 -->
    <section class="home-hero">
      <div class="hero-copy">
        <span class="eyebrow">SmartCommunity Portal</span>
        <h1>智慧社区生活</h1>
        <p>把物业服务、社区购物和日常通知放在同一个入口，居民登录后就能办理常用事项。</p>
        <div class="hero-search">
          <el-input
            v-model.trim="searchKeyword"
            size="large"
            :prefix-icon="Search"
            placeholder="搜索社区商品、日用品、生鲜..."
            @keyup.enter="searchGoods"
          />
          <el-button type="primary" size="large" :icon="Search" @click="searchGoods">搜索</el-button>
        </div>
        <div class="hero-filter-chips">
          <button v-for="item in shoppingChips" :key="item.label" @click="goGoods(item.query)">
            {{ item.label }}
          </button>
        </div>
        <div class="hero-actions">
          <el-button type="primary" size="large" :icon="OfficeBuilding" @click="$router.push('/portal/community')">
            社区服务
          </el-button>
          <el-button size="large" :icon="Goods" @click="$router.push('/portal/goods')">
            浏览商品
          </el-button>
        </div>
      </div>
      <div class="hero-panel">
        <div class="panel-title">今日服务</div>
        <div class="panel-row" v-for="item in heroStats" :key="item.label">
          <el-icon><component :is="item.icon" /></el-icon>
          <span>{{ item.label }}</span>
          <strong>{{ item.value }}</strong>
        </div>
      </div>
    </section>

    <!-- 快捷入口：点击后跳转到社区服务页，并通过 query.tab 自动打开对应业务 tab -->
    <section class="service-strip">
      <button v-for="item in serviceCards" :key="item.title" class="service-card" @click="goService(item.tab)">
        <span class="service-icon">
          <el-icon><component :is="item.icon" /></el-icon>
        </span>
        <span class="service-main">
          <strong>{{ item.title }}</strong>
          <small>{{ item.desc }}</small>
        </span>
        <el-icon class="service-arrow"><ArrowRight /></el-icon>
      </button>
    </section>

    <!-- 分类筛选：顶级分类用按钮，子分类用 tag，适合做商城浏览入口 -->
    <section class="category-band">
      <div class="section-head">
        <div>
          <h2>商品分类</h2>
          <p>按社区生活场景快速筛选商品。</p>
        </div>
        <el-button text :icon="Refresh" @click="goGoods()">全部商品</el-button>
      </div>
      <div class="category-nav">
        <el-button
          v-for="cat in topCategories"
          :key="cat.id"
          :type="activeCategory === cat.id ? 'primary' : 'default'"
          @click="onCategoryClick(cat)"
        >
          {{ cat.categoryName }}
        </el-button>
      </div>
      <div v-if="subCategories.length" class="sub-nav">
        <el-tag
          v-for="sub in subCategories"
          :key="sub.id"
          :type="selectedCategoryId === sub.id ? 'primary' : 'info'"
          class="sub-tag"
          size="large"
          @click="searchByCategory(sub.id)"
        >
          {{ sub.categoryName }}
        </el-tag>
      </div>
    </section>

    <!-- 展示后台配置的第一场有效促销，商品数量由活动的 maxItems 控制。 -->
    <section v-if="specials.length && specialGoods.length" class="section">
      <div class="section-head">
        <div>
          <h2>{{ activeSpecial?.specialName || '限时促销' }}</h2>
          <p>{{ activeSpecial?.specialSubtitle || '社区精选优惠商品。' }}</p>
        </div>
        <el-button text :icon="ArrowRight" @click="goGoods({ promotion: 'true' })">更多优惠</el-button>
      </div>
      <div class="goods-grid compact">
        <div v-for="item in specialGoods" :key="item.goodsNo" class="goods-card promotion-card" @click="goDetail(item)">
          <span class="promotion-badge">{{ item.badgeText || item.promotionLabel }}</span>
          <el-image :src="assetUrl(item.goodsPicture)" fit="cover" class="goods-img">
            <template #error><div class="img-placeholder">暂无图片</div></template>
          </el-image>
          <div class="goods-info">
            <p class="goods-name">{{ item.goodsName }}</p>
            <p class="promotion-meta-line">
              {{ activeSpecial?.specialName || '限时优惠' }} · {{ item.promotionLabel }}
            </p>
            <div class="promotion-action-row" @click.stop>
              <div class="promotion-price-stack">
                <p class="goods-price">{{ item.promotionPrice == null ? '暂无标价' : `¥${item.promotionPrice}` }}</p>
                <del v-if="item.goodsMarketPrice != null">¥{{ item.goodsMarketPrice }}</del>
              </div>
              <el-button
                circle
                text
                type="primary"
                :icon="ShoppingCart"
                :loading="busyKey === `${item.goodsNo}:cart`"
                title="加入购物车"
                @click="quickAddCart(item)"
              />
              <el-button
                circle
                text
                type="primary"
                :icon="Star"
                :loading="busyKey === `${item.goodsNo}:fav`"
                title="收藏"
                @click="quickFavorite(item)"
              />
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- 普通商品列表：和商品列表页共用同一套后端分页接口 -->
    <section class="section">
      <div class="section-head">
        <div>
          <h2>社区好物</h2>
          <p>当前共 {{ total }} 件商品可选。</p>
        </div>
      </div>
      <div class="goods-grid" v-if="goodsList.length">
        <div v-for="item in goodsList" :key="item.goodsNo" class="goods-card" @click="goDetail(item)">
          <span v-if="item.badgeText || item.promotionLabel" class="promotion-badge">
            {{ item.badgeText || item.promotionLabel }}
          </span>
          <el-image :src="assetUrl(item.goodsPicture)" fit="cover" class="goods-img">
            <template #error><div class="img-placeholder">暂无图片</div></template>
          </el-image>
          <div class="goods-info">
            <p class="goods-name">{{ item.goodsName }}</p>
            <p v-if="item.specialName" class="promotion-meta-line">
              {{ item.specialName }} · {{ item.promotionLabel }}
            </p>
            <div class="home-goods-row" @click.stop>
              <div class="home-price-stack">
                <p class="goods-price">¥{{ formatPrice(item.promotionPrice ?? item.goodsMarketPrice) }}</p>
                <del v-if="item.promotionPrice != null && item.goodsMarketPrice != null">¥{{ formatPrice(item.goodsMarketPrice) }}</del>
              </div>
              <el-button
                circle
                text
                type="primary"
                :icon="ShoppingCart"
                :loading="busyKey === `${item.goodsNo}:cart`"
                @click="quickAddCart(item)"
              />
              <el-button
                circle
                text
                type="primary"
                :icon="Star"
                :loading="busyKey === `${item.goodsNo}:fav`"
                @click="quickFavorite(item)"
              />
            </div>
          </div>
        </div>
      </div>
      <div v-else class="page-empty">
        <p>暂无商品</p>
      </div>
      <div class="pagination-wrap" v-if="total > pageSize">
        <el-pagination
          background
          layout="prev, pager, next"
          :pager-count="5"
          :total="total"
          :page-size="pageSize"
          v-model:current-page="currentPage"
          @current-change="loadGoods"
        />
      </div>
    </section>
  </div>
</template>

<script setup>
/**
 * Home - 门户端首页
 *
 * 功能：
 * - Hero 区域：品牌展示、搜索入口、快捷筛选标签、今日服务面板
 * - 社区服务快捷入口卡片（点击跳转到社区服务页对应 tab）
 * - 商品分类导航（一级分类按钮 + 二级分类标签）
 * - 限时促销商品展示（自动选取数据最完整的活动）
 * - 普通商品分页列表（支持快速加入购物车和收藏）
 *
 * 路由路径：/portal
 */

import { ref, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import {
  ArrowRight,
  Bell,
  Document,
  Goods,
  HomeFilled,
  Key,
  OfficeBuilding,
  Refresh,
  Search,
  ShoppingCart,
  Star,
  Van,
  Wallet
} from '@element-plus/icons-vue'
import { addFavorite, addToCart, getGoodsPage, getCategories, getGoodsStores, getSpecialList, getSpecialGoods } from '@/portal/api'
import { usePortalAuthStore } from '@/portal/stores/auth'
import { assetUrl } from '@/utils/asset'

// ========== 路由实例 ==========
const router = useRouter()
// ========== 认证状态 ==========
const auth = usePortalAuthStore()

// ========== 响应式状态 ==========
const topCategories = ref([])           // 一级分类列表
const subCategories = ref([])           // 当前选中一级分类下的子分类列表
const specials = ref([])                // 所有促销活动列表
const specialGoods = ref([])            // 当前展示的促销商品列表
const activeSpecial = ref(null)         // 当前选中的促销活动对象
const goodsList = ref([])               // 普通商品分页列表数据
const currentPage = ref(1)              // 商品列表当前页码
const pageSize = ref(12)                // 商品列表每页条数
const total = ref(0)                    // 商品列表总数
const selectedCategoryId = ref(null)    // 当前选中的分类 ID（用于筛选）
const activeCategory = ref(null)        // 当前激活的一级分类 ID
const searchKeyword = ref('')           // 搜索关键词
const busyKey = ref('')                 // 正在操作的商品标识（用于按钮 loading 状态控制）

// 顶部右侧”今日服务”面板的数据，只做展示，不依赖后端。
const heroStats = [
  { label: '物业公告', value: '实时', icon: Bell },
  { label: '报修工单', value: '在线提交', icon: Document },
  { label: '社区配送', value: '到家', icon: Van }
]

// 首页服务卡片配置。以后要新增入口，只需要往数组里加一项。
const serviceCards = [
  { title: '通知公告', desc: '查看社区最新通知', tab: 'notice', icon: Bell },
  { title: '报事报修', desc: '问题在线提交跟进', tab: 'complaint', icon: Document },
  { title: '访客登记', desc: '亲友来访提前登记', tab: 'visitor', icon: Key },
  { title: '物业缴费', desc: '账单明细一键查看', tab: 'charge', icon: Wallet },
  { title: '我的车位', desc: '车位与车辆绑定信息', tab: 'parking', icon: HomeFilled },
  { title: '购物车', desc: '继续处理待购商品', tab: 'cart', icon: ShoppingCart }
]

// 快捷筛选标签配置
const shoppingChips = [
  { label: '限时优惠', query: { promotion: 'true' } },
  { label: '只看有货', query: { hasStock: 'true' } },
  { label: '30元内好物', query: { minPrice: 0, maxPrice: 30 } },
  { label: '最新上架', query: { sort: 'newest' } },
  { label: '价格从低到高', query: { sort: 'priceAsc' } }
]

// ========== 生命周期 ==========
onMounted(async () => {
  // Promise.allSettled 的好处：某个接口失败时，不影响其它区域继续渲染。
  const [catRes] = await Promise.allSettled([
    getCategories(),
    loadGoods(),
    loadSpecials()
  ])
  if (catRes.status === 'fulfilled' && catRes.value?.data) {
    const all = catRes.value.data
    // 筛选一级分类：categoryType === 1 或没有 parentId 的为顶级分类
    topCategories.value = all.filter(c => c.categoryType === 1 || !c.parentId)
  }
})

/**
 * 加载商品分页
 * 分类筛选时会带 categoryId，不筛选时传 undefined
 */
async function loadGoods() {
  try {
    const res = await getGoodsPage({
      page: currentPage.value,
      size: pageSize.value,
      categoryId: selectedCategoryId.value || undefined
    })
    if (res?.data) {
      goodsList.value = res.data.records || []
      total.value = res.data.total || 0
    }
  } catch (e) { /* 错误由统一拦截器处理 */ }
}

/**
 * 加载促销活动及商品
 * 后端已经过滤未开始、已结束和已停用活动，这里取排序最靠前的一场
 */
async function loadSpecials() {
  try {
    const res = await getSpecialList()
    if (res?.data) {
      specials.value = res.data
      if (!res.data.length) return

      // 首页优先展示”优惠价和图片数据完整”的活动，避免旧活动商品未标价时出现空白卡片。
      const bundles = await Promise.all(res.data.map(async special => {
        const goodsResponse = await getSpecialGoods(special.id)
        const goods = goodsResponse?.data || []
        const completeCount = goods.filter(item => item.promotionPrice != null && item.goodsPicture).length
        return { special, goods, completeCount }
      }))
      bundles.sort((a, b) => b.completeCount - a.completeCount)
      const selected = bundles[0]
      activeSpecial.value = selected?.special || res.data[0]
      const maxItems = Math.max(1, Math.min(Number(activeSpecial.value?.maxItems || 4), 20))
      // maxItems 只控制首页展示数量；优惠商品查询接口会返回活动下全部有效商品。
      specialGoods.value = (selected?.goods || []).slice(0, maxItems)
    }
  } catch (e) { /* 错误已处理 */ }
}

/**
 * 点击一级分类：先查找其子分类，若无子分类则直接按该分类查询商品
 * @param {Object} cat - 分类对象，包含 id 等属性
 */
function onCategoryClick(cat) {
  activeCategory.value = cat.id
  subCategories.value = []
  getCategories().then(res => {
    if (res?.data) {
      subCategories.value = res.data.filter(c => c.parentId === cat.id)
      // 如果没有子分类，直接按一级分类筛选商品
      if (!subCategories.value.length) {
        searchByCategory(cat.id)
      }
    }
  })
}

/**
 * 按子分类筛选商品
 * @param {number|string} categoryId - 分类 ID
 */
function searchByCategory(categoryId) {
  selectedCategoryId.value = categoryId
  currentPage.value = 1
  loadGoods()
}

/**
 * 清空分类筛选，回到全部商品
 */
function resetCategory() {
  activeCategory.value = null
  selectedCategoryId.value = null
  subCategories.value = []
  currentPage.value = 1
  loadGoods()
}

/**
 * 社区服务入口跳转
 * @param {string} tab - 目标 Tab 名称，'cart' 特殊处理为跳转购物车页面
 */
function goService(tab) {
  if (tab === 'cart') {
    router.push('/portal/cart')
    return
  }
  router.push({ path: '/portal/community', query: { tab } })
}

/**
 * 跳转到商品列表页
 * @param {Object} query - 查询参数对象
 */
function goGoods(query = {}) {
  router.push({ name: 'GoodsList', query })
}

/**
 * 执行搜索：有关键词则跳转商品列表页并带关键词参数
 */
function searchGoods() {
  if (!searchKeyword.value) {
    goGoods()
    return
  }
  goGoods({ keyword: searchKeyword.value })
}

/**
 * 跳转到商品详情页
 * @param {Object} item - 商品对象
 */
function goDetail(item) {
  router.push({ name: 'GoodsDetail', params: { goodsNo: item.goodsNo } })
}

/**
 * 格式化价格显示
 * @param {number|null} value - 价格数值
 * @returns {string} 格式化后的价格字符串
 */
function formatPrice(value) {
  return value == null ? '暂无标价' : Number(value).toFixed(2)
}

/**
 * 快速加入购物车（需登录）
 * 先查找有库存的门店，再调用加入购物车接口
 * @param {Object} item - 商品对象
 */
async function quickAddCart(item) {
  if (!auth.isLoggedIn) {
    return router.push({ path: '/portal/login', query: { redirect: '/portal' } })
  }
  busyKey.value = `${item.goodsNo}:cart`
  try {
    const store = await firstAvailableStore(item.goodsNo)
    if (!store) return
    await addToCart({ goodsNo: item.goodsNo, storeNo: store.storeNo, amount: 1 })
  } finally {
    busyKey.value = ''
  }
}

/**
 * 快速收藏商品（需登录）
 * @param {Object} item - 商品对象
 */
async function quickFavorite(item) {
  if (!auth.isLoggedIn) {
    return router.push({ path: '/portal/login', query: { redirect: '/portal' } })
  }
  busyKey.value = `${item.goodsNo}:fav`
  try {
    await addFavorite(item.goodsNo)
  } finally {
    busyKey.value = ''
  }
}

/**
 * 查找商品第一个有库存的门店
 * @param {string} goodsNo - 商品编号
 * @returns {Object|undefined} 有库存的门店对象，若无则返回第一个门店
 */
async function firstAvailableStore(goodsNo) {
  const res = await getGoodsStores(goodsNo)
  const stores = res?.data || []
  return stores.find(item => Number(item.goodsStock || 0) > 0) || stores[0]
}
</script>

<style scoped>
.home {
  display: flex;
  flex-direction: column;
  gap: 24px;
}

.home-hero {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 340px;
  gap: 24px;
  min-height: 260px;
  padding: 34px;
  color: #fff;
  background:
    linear-gradient(110deg, rgba(9, 31, 52, 0.88), rgba(15, 108, 189, 0.68), rgba(0, 120, 128, 0.42)),
    url('/smartcommunitybackground.webp') center/cover;
  border-radius: var(--radius);
  border: 1px solid rgba(255, 255, 255, 0.34);
  box-shadow: var(--shadow-lg);
  overflow: hidden;
}

.hero-copy {
  display: flex;
  flex-direction: column;
  justify-content: center;
  max-width: 620px;
}

.eyebrow {
  font-size: 13px;
  letter-spacing: 0;
  color: rgba(255, 255, 255, 0.72);
  margin-bottom: 8px;
}

.hero-copy h1 {
  color: #fff;
  font-size: 42px;
  line-height: 1.12;
  margin: 0 0 14px;
  letter-spacing: 0;
}

.hero-copy p {
  color: rgba(255, 255, 255, 0.82);
  font-size: 16px;
  max-width: 520px;
}

.hero-actions {
  display: flex;
  gap: 12px;
  flex-wrap: wrap;
  margin-top: 26px;
}

.hero-panel {
  align-self: center;
  background: rgba(240, 248, 255, 0.16);
  border: 1px solid rgba(255, 255, 255, 0.34);
  backdrop-filter: blur(22px) saturate(1.25);
  -webkit-backdrop-filter: blur(22px) saturate(1.25);
  box-shadow: 0 12px 30px rgba(4, 23, 39, 0.14), 0 1px 0 rgba(255, 255, 255, 0.26) inset;
  border-radius: var(--radius);
  padding: 18px;
}

.panel-title {
  color: rgba(255, 255, 255, 0.72);
  font-size: 13px;
  margin-bottom: 12px;
}

.panel-row {
  display: grid;
  grid-template-columns: 24px 1fr auto;
  gap: 10px;
  align-items: center;
  min-height: 38px;
  color: rgba(255, 255, 255, 0.86);
}

.panel-row strong {
  color: #fff;
  font-weight: 600;
}

.service-strip {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 14px;
}

.service-card {
  display: grid;
  grid-template-columns: 42px minmax(0, 1fr) 20px;
  align-items: center;
  gap: 12px;
  min-height: 86px;
  padding: 16px;
  border: 1px solid var(--border-light);
  border-radius: var(--radius);
  background: var(--surface);
  box-shadow: var(--shadow-sm);
  backdrop-filter: blur(18px) saturate(1.2);
  color: var(--text);
  cursor: pointer;
  text-align: left;
  transition: transform var(--motion), box-shadow var(--motion), border-color var(--motion);
}

.service-card:hover {
  border-color: var(--accent-border);
  box-shadow: var(--shadow-md);
  transform: translateY(-2px);
}

.service-icon {
  width: 42px;
  height: 42px;
  border-radius: var(--radius);
  display: inline-flex;
  align-items: center;
  justify-content: center;
  background: var(--accent-bg);
  color: var(--accent);
  font-size: 20px;
}

.service-main {
  min-width: 0;
}

.service-main strong,
.service-main small {
  display: block;
}

.service-main strong {
  color: var(--text-h);
  font-size: 15px;
}

.service-main small {
  color: var(--text);
  margin-top: 2px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.service-arrow {
  color: var(--text);
}

.category-band,
.section {
  border-radius: var(--radius);
}

.category-band {
  padding: 20px;
  background: var(--surface);
  border: 1px solid var(--border-light);
  box-shadow: var(--shadow-sm);
  backdrop-filter: blur(18px) saturate(1.2);
}

.section {
  padding: 4px 0;
  background: transparent;
  border: 0;
}

.section-head {
  display: flex;
  align-items: flex-start;
  justify-content: space-between;
  gap: 14px;
  margin-bottom: 16px;
}

.section-head h2 {
  margin-bottom: 4px;
}

.category-nav {
  display: flex;
  gap: 10px;
  flex-wrap: wrap;
}

.sub-nav {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
  margin-top: 14px;
}

.sub-tag {
  cursor: pointer;
}

.goods-grid.compact .goods-img {
  height: 180px;
}

.hero-search {
  display: flex;
  gap: 10px;
  max-width: 620px;
  margin-top: 24px;
}

.hero-search :deep(.el-input__wrapper) {
  border: 1px solid rgba(255, 255, 255, 0.36);
  background: rgba(255, 255, 255, 0.92);
  box-shadow: 0 8px 22px rgba(4, 23, 39, 0.12);
}

.hero-filter-chips {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  margin-top: 12px;
}

.hero-filter-chips button {
  border: 1px solid rgba(255, 255, 255, 0.34);
  border-radius: 999px;
  padding: 6px 12px;
  background: rgba(255, 255, 255, 0.16);
  color: rgba(255, 255, 255, 0.9);
  cursor: pointer;
}

.promotion-card {
  position: relative;
}

.promotion-badge {
  position: absolute;
  z-index: 2;
  top: 10px;
  left: 10px;
  max-width: calc(100% - 20px);
  padding: 3px 8px;
  overflow: hidden;
  border-radius: 4px;
  color: #fff;
  background: #c42b1c;
  box-shadow: 0 2px 8px rgba(196, 43, 28, 0.24);
  font-size: 12px;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.promotion-meta-line {
  margin-top: 8px;
  color: #c42b1c;
  font-size: 13px;
  font-weight: 700;
}

.promotion-action-row {
  display: flex;
  align-items: center;
  gap: 10px;
  margin-top: 8px;
}

.promotion-price-stack {
  flex: 1;
  min-width: 0;
}

.promotion-price-stack .goods-price {
  color: #c42b1c;
  font-size: 20px;
  font-weight: 800;
}

.promotion-price-stack del {
  display: block;
  margin-top: 4px;
  color: var(--text-soft);
  font-size: 13px;
}

.home-goods-row {
  display: flex;
  align-items: center;
  gap: 8px;
}

.home-goods-row .goods-price {
  min-width: 0;
}

.home-price-stack {
  flex: 1;
  min-width: 0;
}

.home-price-stack del {
  display: block;
  margin-top: 3px;
  color: var(--text-soft);
  font-size: 12px;
}

@media (max-width: 1024px) {
  .home-hero {
    grid-template-columns: 1fr;
    padding: 24px;
  }

  .hero-copy h1 {
    font-size: 32px;
  }

  .service-strip {
    grid-template-columns: 1fr;
  }

  .hero-search {
    flex-direction: column;
  }
}
</style>

