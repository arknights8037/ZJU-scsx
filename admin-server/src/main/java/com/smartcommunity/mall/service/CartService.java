...
public class CartService {
     ...
     * 否则插入一条新购物车记录。
     */
    public void add(Integer userId, Carts cart) {
        if (cart == null || cart.getGoodsNo() == null || cart.getStoreNo() == null) {
    ...
    /**
     * 兼容老师示例工程的旧路径。新居民端接口不要使用这个重载。
     */
    public void add(Carts cart) {
        if (cart == null || cart.getUserId() == null) {
            throw new RuntimeException("缺少用户信息");
        }
        add(cart.getUserId(), cart);
    }

    /**
     * 修改数量。数量异常时兜底成 1，避免出现 0 件或负数。
     */
    public void updateAmount(Integer userId, Integer id, Integer amount) {
    ...
    public void updateAmount(Integer id, Integer amount) {
        ...
        updateAmount(cart.getUserId(), id, amount);
    }
    ...
    private void fillDetails(List<Carts> carts) {
        ...
        carts.forEach(cart -> {
            cart.setGoods(specialService.enrichGoods(goodsMap.get(cart.getGoodsNo())));
            cart.setStore(storeMap.get(cart.getStoreNo()));
            GoodsStore goodsStore = goodsStoreMap.get(goodsStoreKey(cart.getStoreNo(), cart.getGoodsNo()));
            if (goodsStore != null) {
                goodsStore.setGoodsPrice(specialService.effectivePrice(cart.getGoodsNo(), goodsStore.getGoodsPrice()));
            }
            cart.setGoodsStore(goodsStore);
        });
    }
    ...
    /**
     * 按 storeNo 和 goodsNo 生成 Map 的复合键。
     *
     * @param storeNo 门店编号
     * @param goodsNo 商品编号
     * @return 复合键字符串（storeNo:goodsNo）
     */
