...
- **mall-portal-server** `src/main/resources/application.yml` → 默认连接 `mall_db`
...
居民头像和商品主图默认分别保存在两个后端模块自己的 `uploads/` 目录中，该目录已加入
`.gitignore`，不会把用户上传文件提交到 Git。

部署时可通过环境变量修改保存位置：

- `MALL_UPLOAD_DIR`：居民头像目录，对外路径为 `/mall-uploads/**`
- `ADMIN_UPLOAD_DIR`：商品图片目录，对外路径为 `/admin-uploads/**`
